...
function renderHeader(activePage) {
  ...
  var logoSrc = prefix + 'images/logo.png?v=61';
    ...
    fetch('https://formspree.io/f/mzdypypl', {
      ...
      if (wrap) {
        wrap.innerHTML = '<div style="text-align:center;padding:20px">' +
          ...
          '<p style="color:#9ca3af;font-size:13px">Market updates will be sent to ' + email + '</p></div>';
      }
    }).catch(function() {
      if (wrap) {
        wrap.innerHTML = '<div style="text-align:center;padding:20px">' +
...
function renderFooter() {
  const inSub = window.location.pathname.includes('/category/') || window.location.pathname.includes('/authors/') || window.location.pathname.includes('/articles/');
  const p = inSub ? '../' : '';
  const footerLogoMarkup = `<img src="${p}images/logo.png?v=61" alt="EnergyPricesToday.com" class="footer-logo-img">`;
