#!/usr/bin/env python3
"""
Comprehensive correction-notice pass.

For articles where the content body is heavily contaminated with unverifiable
named-source quotes, replace the entire article file with a correction notice
that preserves the URL (avoiding 404s) but clearly states the content has been
removed pending verification.

For articles where contamination is contained to one or two sentences and the
rest of the content is verifiable, do targeted sentence removal.
"""
...
# Articles to convert to correction notices because contamination is deep
HEAVY_CONTAMINATION = [
    ("trump-heads-to-beijing-set-to-press-xi-to-lean-on-iran.html",
     "It contained material attributed to a named analyst at a geopolitical-risk firm and to a former U.S. energy advisor that EnergyPricesToday Editorial could not independently verify."),
    ("trump-weighs-return-to-military-action-national-security-team-hormuz.html",
     "It contained material attributed to multiple named analysts and a named Iranian military spokesperson that EnergyPricesToday Editorial could not independently verify."),
    ("wti-tops-102-brent-108-as-hormuz-stalemate-deepens.html",
     "It contained material attributed to a named oil-company executive, a named former U.S. energy advisor, and to a major investment bank that EnergyPricesToday Editorial could not independently verify."),
    ("oil-rallies-5-percent-as-hormuz-deal-collapses-wti-100-brent-106.html",
     "It contained material attributed to a named oil-company executive that EnergyPricesToday Editorial could not independently verify."),
    ...
    ("us-gas-average-surges-to-439-up-25-cents-in-three-days.html",
     "It contained material attributed to a named analyst at a fuel-tracking firm that EnergyPricesToday Editorial could not independently verify in subsequent review."),
    ("oil-crashes-13-percent-intraday-as-us-iran-mou-talks-emerge.html",
     "It contained material citing prediction-market positioning that EnergyPricesToday Editorial could not independently verify."),
    ("iran-mohsen-rezaei-demands-us-reparations-before-any-deal.html",
     "It contained material attributed to a named Iranian political figure that EnergyPricesToday Editorial could not independently verify."),
    ("trump-rejects-iran-counterproposal-as-totally-unacceptable-threatens-bombing.html",
     "It contained material attributed to a named Iranian military spokesperson and a named oil-company executive that EnergyPricesToday Editorial could not independently verify."),
]
...
def main():
    for slug, reason in HEAVY_CONTAMINATION:
        path = ARTICLES / slug
        if not path.exists():
        ...
        print(f"  ✓ {slug}")
    print(f"\n{len(HEAVY_CONTAMINATION)} articles replaced with correction notices.")
