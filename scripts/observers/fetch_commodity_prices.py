...
def normalize_api_response(raw: dict) -> dict:
    ...
    The real shape (as of April 2026) is:
      {
        "status": "success",
        "data": {
          "status": "success",
          "data": {
            "prices": {
              "WTI_USD": {
                ...
                "price": 98.12,
                "currency": "USD",
                "updated_at": "2026-04-18T22:00:04Z",
                "change_24h": 0.0,
    ...
    The prices object is keyed by source code, NOT a list. Change field is
    `change_24h` (absolute) and `change_24h_percent` (percentage).
    """
    prices = {}
    if not isinstance(raw, dict):
        return prices

    # Navigate: raw.data.data.prices (handle both 1-level and 2-level nesting defensively)
    node = raw.get("data", {})
    ...
    for code, item in price_map.items():
        ...
        try:
            ...
            pct = float(item.get("change_24h_percent", 0) or 0)
